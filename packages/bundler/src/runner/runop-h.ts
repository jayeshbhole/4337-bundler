...
import { HumanAccountFactory__factory } from '@humanwallet/contracts'
import { formatEther, keccak256, parseEther, defaultAbiCoder, arrayify } from 'ethers/lib/utils'
import { Command } from 'commander'
import { erc4337RuntimeVersion } from '@account-abstraction/utils'
import fs from 'fs'
import { DeterministicDeployer, HttpRpcClient, HumanAccountAPI } from '@humanwallet/sdk'
...
class Runner {
  ...
  async init (deploymentSigner?: Signer): Promise<this> {
    ...
    const accountDeployer = await DeterministicDeployer.getAddress(new HumanAccountFactory__factory(), 0, [this.entryPointAddress])
    // const accountDeployer = await new HumanAccountFactory__factory(this.provider.getSigner()).deploy().then(d=>d.address)
    ...
    if (!await dep.isContractDeployed(accountDeployer)) {
      ...
      await dep1.deterministicDeploy(new HumanAccountFactory__factory(), 0, [this.entryPointAddress])
    }
    this.bundlerProvider = new HttpRpcClient(this.bundlerUrl, this.entryPointAddress, chainId)
    this.accountApi = new HumanAccountAPI({
      provider: this.provider,
      signer: this.accountOwner,
...
async function main (): Promise<void> {
  ...
  const ownerClient = await new Runner(provider, opts.bundlerUrl, accountOwner, opts.entryPoint, index).init(deployFactory ? signer : undefined)

  const humanAccountAddr = await ownerClient.getAddress()
  ...
  const bal = await getBalance(humanAccountAddr)
  console.log('account address', humanAccountAddr, 'deployed=', await isDeployed(humanAccountAddr), 'bal=', formatEther(bal))
  ...
  // register device key if not registered
  const humanAccountContract = await ownerClient.accountApi._getAccountContract()
  const deviceSigner = provider.getSigner(1)
  const devicePubKey = await deviceSigner.getAddress()

  // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
  if (!(await humanAccountContract.deviceKeys(devicePubKey))) {
    console.log('===registering device key', devicePubKey)
    const registerRequestHash = keccak256(
      defaultAbiCoder.encode(['address'], [devicePubKey])
    )
    // sign the public key with the account's private key
    const sig = await accountOwner.signMessage(arrayify(registerRequestHash))

    // register the public key with the smart contract account
    const registerKey1Tx = await humanAccountContract.registerDeviceKey(devicePubKey, sig)
    await registerKey1Tx.wait()
  }

  const deviceClient = await new Runner(provider, opts.bundlerUrl, deviceSigner, opts.entryPoint, index).init(deployFactory ? signer : undefined)

  const dest = humanAccountAddr
  const data = keccak256(Buffer.from('nonce()')).slice(0, 10)
  console.log('data=', data)
  await deviceClient.runUserOp(dest, data)
